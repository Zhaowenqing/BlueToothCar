...
#define UART_BASE 0xB0501000
#define CAR_DRT_BASE 0xB0800000
#define CAR_SPD_BASE 0xB0700000
...
static uint32_t sys_hello(uint32_t arg[]){
    kprintf("kernel:hello world!\n\r");
    kprintf("kernel:hello world!\n\r");
    return 0;
}
static uint32_t sys_blt(uint32_t arg[]){
    volatile unsigned int j;
    *WRITE_IO(UART_BASE + 12) = 0x00000080;
    for (j = 0; j < (1000); j++) ;
	*WRITE_IO(UART_BASE + 0) = 70; 
    for (j = 0; j < (1000); j++) ;
	*WRITE_IO(UART_BASE + 4) = 1;
    for (j = 0; j < (1000); j++) ;
	*WRITE_IO(UART_BASE + 12) = 0x00000003;
    for (j = 0; j < (1000); j++) ;  
	*WRITE_IO(UART_BASE + 4) = 0x00000000; 
	*WRITE_IO(UART_BASE + 8)= 0x000000c1;
    return 0; 
}
static uint32_t sys_rblt(uint32_t arg[]){
     while(!((*READ_IO(UART_BASE + 20) & 0x00000001)==0x00000001));
        return (uint32_t)*READ_IO(UART_BASE + 0);
}

static uint32_t sys_pmd(uint32_t arg[]){
    int fd1=(int)arg[0];
    int fd2=(int)arg[1];
    *WRITE_IO(CAR_DRT_BASE)=fd1;
	*WRITE_IO(CAR_SPD_BASE)=fd2;
    return 0;
}
